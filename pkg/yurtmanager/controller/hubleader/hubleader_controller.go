...
func init() {
	flag.IntVar(
		...
		"hubleader-workers",
		concurrentReconciles,
		"Max concurrent workers for HubLeader controller.",
	)
}
...
func Add(ctx context.Context, cfg *appconfig.CompletedConfig, mgr manager.Manager) error {
	...
	poolPredicate := predicate.Funcs{
		...
		UpdateFunc: func(e event.UpdateEvent) bool {
			...
			// 3. Leader node label selector has changed (if mark strategy)
...
// +kubebuilder:rbac:groups=apps.openyurt.io,resources=nodepool,verbs=get;list;watch;update;patch
...
func (r *ReconcileHubLeader) reconcileHubLeader(ctx context.Context, nodepool *appsv1beta2.NodePool) error {
	...
	// If there is no leader, or number of leaders are less than desired elect a new leader
	if len(updatedLeaders) == 0 ||
		len(updatedLeaders) < int(nodepool.Spec.LeaderReplicas) &&
			nodepool.Spec.LeaderElectionStrategy == string(appsv1beta2.ElectionStrategyMark) {
		// No leader, it should elect a new leader
		leaders, ok := electLeaders(nodepool.Spec.LeaderElectionStrategy, nodepool.Spec.LeaderReplicas, endpointsMap)
		if !ok {
...
func electLeaders(strategy string, leaderReplicas int32, candidates map[string]*corev1.Node) ([]string, bool) {
	leaderEndpoints := make([]string, 0, len(candidates))

	numLeaders := int(leaderReplicas)
	if len(candidates) < numLeaders {
		// Not enough candidates to elect desired leaders
		// Set numLeaders to the number of candidates
		numLeaders = len(candidates)
	}

	switch strategy {
	case string(appsv1beta2.ElectionStrategyMark):
	...
	case string(appsv1beta2.ElectionStrategyRandom):
		// Randomly select a leader node
		// Iterating slice in Go is non-deterministic enough to be considered random
		// for this purpose
